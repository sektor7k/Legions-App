...
export default function OutboxTab() {
  ...
  const { data: invites, error, mutate } = useSWR(
    session?.user?.id
      ? [
          `${process.env.NEXT_PUBLIC_API_URL}/tournament/invite/getOutbox`,
    ...
    <div className="space-y-4 overflow-y-auto max-h-96 scrollbar-hide">
      {invites.length === 0 ? (
        ...
        invites.map((invite: any, index: number) => (
            <div
            key={index}
            className="p-6 rounded-lg bg-gray-800 border border-gray-900 bg-opacity-40 backdrop-blur-sm mb-4 shadow-lg transition transform hover:-translate-y-1 hover:shadow-xl"
          >
