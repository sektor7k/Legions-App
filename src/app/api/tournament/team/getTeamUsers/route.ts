...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        const teams = await Team.find({ tournamentId, isDeleted: false })
            .populate({
                path: 'members.memberId',
                select: 'username image email wallets.evm wallets.solana socialMedia.twitter socialMedia.discord socialMedia.telegram' // Sadece kullanıcı adı ve resmi al
            });
